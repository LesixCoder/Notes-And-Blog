...
> 源于自己对 DOM, js 的理解
>
> 已实现, 类似`on(), off(), css(), addClass()`等三十几种常用方法
...
(function(w) {
  ...
  jQuery.extend({
    ...
    isLikeArray(selector) {
      return (
        (!!selector && 'length in selector' && selector.length === 0) ||
    ...
    isDom(obj = {}) {
      try {
        return (
          (obj instanceof NodeList || obj instanceof HTMLElement) &&
          obj.nodeType === 1 &&
          obj.nodeValue === 'null' &&
    ...
    ajax(option = {}) {
      ...
      const { method, url, data, success, error, complete } = {
  ...
  jQuery.fn.extend({
      ...
      try {
        ...
        // content => 字符串
          ...
          if (jQuery.isHtmlPart(content)) {
            ...
            for (let i = 0, outerLen = this.length; i < outerLen; i++) {
              for (
                let innerLen = content.length, j = innerLen - 1;
                j >= 0;
                j--
              ) {
                let clone = content[j].cloneNode(true);
        ...
        if (jQuery.isString(param)) {
          if (this.length === 1) {
            ...
            for (let i = 0, len = this.length; i < len; i++) {
              valueMap.set(
                this[i],
                window.getComputedStyle(this.get(i), null)[param]
              );
            }
    ...
    on(...args) {
      ...
      **/
      ...
      // three param
        ...
        try {
          const [type, child, handler, selector] = [
            jQuery.trim(args[0]),
            jQuery.trim(args[1]),
            args[2],
            jQuery.isWhichSelector(jQuery.trim(args[1]))
          ];

          if (
            jQuery.isString(type) &&
            jQuery.isString(child) &&
            jQuery.isFunction(handler)
          ) {
            // 如果为 多个event
            if (regOneEvent.test(type)) {
              const saveEvent = type.split(' ');
              ...
              if (jQuery.isLikeArray(selector)) {
                ...
                const [parent, thisName] = [
                  selector[0].parentNode.localName ||
                    selector[0].parentElement.localName,
                  this.get(0).localName
                ];
                  ...
                  for (let k of saveEvent) {
                    for (let i = 0, every; (every = this[i++]); ) {
                      $.addEvent(k, every, function(e) {
                        ...
                        Object.is(e.target.localName, selector[0].localName) &&
                          handler();
                      });
                    }
                  }
    ...
    siblings(...args) {
      ...
      **/
      ...
      if (Object.is(args.length, 0)) {
        ...
        let child = [
          ...(element.parentElement.children || element.parentNode.childNodes)
        ];
    ...
    next(ele) {
      ...
      for (let i = 0, every; (every = this[i++]); ) {
        (every.nextElementSibling || every.nextSibling !== null) &&
    ...
    nextAll(ele) {
      ...
      const element = this.get(0);
      let child = [
        ...(element.parentElement.children || element.parentNode.chlidNodes)
      ];
      let index = child.indexOf(element);
